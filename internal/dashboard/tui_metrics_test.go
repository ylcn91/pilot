...
func TestHydrateFromStore_LifetimeTokens(t *testing.T) {
	...
	if m.metrics.card.InputTokens != wantInput {
		t.Errorf("InputTokens = %d, want %d", m.metrics.card.InputTokens, wantInput)
	}
	if m.metrics.card.OutputTokens != wantOutput {
		t.Errorf("OutputTokens = %d, want %d", m.metrics.card.OutputTokens, wantOutput)
	}
	if m.metrics.card.TotalTokens != wantTotal {
		t.Errorf("TotalTokens = %d, want %d", m.metrics.card.TotalTokens, wantTotal)
	}
	if math.Abs(m.metrics.card.TotalCostUSD-wantCost) > 0.001 {
		t.Errorf("TotalCostUSD = %.4f, want %.4f", m.metrics.card.TotalCostUSD, wantCost)
	}
}
...
func TestUpdateTokensMsg_AddsToLifetimeTotals(t *testing.T) {
	...
	if model.metrics.card.InputTokens != 12000 {
		t.Errorf("InputTokens = %d, want 12000", model.metrics.card.InputTokens)
	}
	if model.metrics.card.OutputTokens != 6000 {
		t.Errorf("OutputTokens = %d, want 6000", model.metrics.card.OutputTokens)
	}
	if model.metrics.card.TotalTokens != 18000 {
		t.Errorf("TotalTokens = %d, want 18000", model.metrics.card.TotalTokens)
	}
}
...
func TestStoreRefreshMsg_UpdatesHistoryAndMetrics(t *testing.T) {
	...
	m.metrics.card = MetricsCardData{TotalTasks: 1, Failed: 1}
	...
	if model.metrics.card.TotalTasks != 2 {
		t.Errorf("TotalTasks = %d, want 2", model.metrics.card.TotalTasks)
	}
	if model.metrics.card.Failed != 0 {
		t.Errorf("Failed = %d, want 0", model.metrics.card.Failed)
	}
}
...
func TestUpdateTokens_ModelAwareCost(t *testing.T) {
	...
	if sonnetModel.metrics.card.TotalCostUSD != wantSonnet {
		t.Errorf("Sonnet cost = %.4f, want %.4f", sonnetModel.metrics.card.TotalCostUSD, wantSonnet)
	}
	...
	if opusModel.metrics.card.TotalCostUSD != wantOpus {
		t.Errorf("Opus cost = %.4f, want %.4f", opusModel.metrics.card.TotalCostUSD, wantOpus)
	}
...
func TestUpdateTokens_EmptyModelFallsBackToDefault(t *testing.T) {
	...
	if model.metrics.card.TotalCostUSD != want {
		t.Errorf("cost with empty model = %.6f, want %.6f (DefaultModel=%s)", model.metrics.card.TotalCostUSD, want, memory.DefaultModel)
	}
}

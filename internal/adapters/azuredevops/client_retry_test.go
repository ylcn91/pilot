...
func TestClient_doRequest_RetriesTransient(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "504 then success",
			maxRetries: 3,
			script: []scriptedStatus{
				{code: http.StatusGatewayTimeout, body: `timeout`},
				{code: http.StatusOK, body: `{"id":1}`},
			},
			wantAttempts: 2,
		},
		{
			...
			maxRetries:   3,
			script:       []scriptedStatus{{code: http.StatusNotFound, body: `not found`}},
			wantErr:      true,
			wantAttempts: 1,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := client.doRequest(context.Background(), http.MethodGet, "/_apis/wit/workitems/1", nil, nil)

...
func (c *Controller) ScanRecentlyMergedPRs(ctx context.Context) error {
	// Run the scan unconditionally: it covers self-heal and merge metrics even in
	// plain GitHub issue-source deployments with neither release nor board sync
	// enabled. Internal gates below handle release triggering and board write-back.

---
title: Configuration
description: Configure Pilot through its YAML config file at ~/.pilot/config.yaml, with environment variable expansion and the interactive setup wizard.
---
...
| `create_sub_issues` | bool | `false` | Create tracker sub-issues from epic planning. When false, planned epics execute as one task |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `claude_code.allowed_tools` | []string | `["Read","Write","Edit","Bash","Grep","Glob","Task"]` | Whitelist passed via `--allowedTools`. Excludes MCP and Web* tools by default to cut per-turn context bloat ([GH-2432](https://github.com/ylcn91/pilot/issues/2432)). |
...
orchestrator:
  autopilot:
    ...
    auto_create_issues: false             # opt-in: create fix issues on CI failure
    issue_labels:
      - pilot
      - autopilot-fix
...
| `auto_create_issues` | bool | `false` | Create fix issues when CI fails |
